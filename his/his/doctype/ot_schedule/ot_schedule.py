...
class OTSchedule(Document):
	@frappe.whitelist()
	def admit(self,patient,service_unit_type=None, expected_discharge=None):
		service_unit = frappe.db.get_value("Healthcare Service Unit",{"service_unit_type":service_unit_type},"name")
		if service_unit:
			type = frappe.db.get_value("Healthcare Service Unit", service_unit, "service_unit_type")
			frappe.db.set_value("Healthcare Service Unit", service_unit, "occupancy_status", "Occupied")
			frappe.db.set_value("Healthcare Service Unit", service_unit, "patient", self.patient_name)
			frappe.db.set_value("Healthcare Service Unit Type", service_unit_type, "patient", patient)
			frappe.db.set_value("Healthcare Service Unit Type", service_unit_type, "patient_name", self.patient_name)
			frappe.errprint(patient)
			clpro = frappe.get_doc({
			'doctype': 'Clinical Procedure',
			'patient': self.patient,
			"procedure_template": self.procedure_template,
			"ot_schedule": self.name,
	...
	def update_prescription_details(self):
		if self.procedure_prescription:
			...
			if self.procedure_template:
				...
				if comments:
					frappe.db.set_value("OT Schedule", self.name, "notes", comments)
...
def invoice_appointment(appointment_doc):
	...
	appointment_invoiced = frappe.db.get_value(
		"OT Schedule", appointment_doc.name, "invoiced"
	)
...
def check_is_new_patient(patient, name=None):
	...
	has_previous_appointment = frappe.db.exists("OT Schedule", filters)
...
def get_appointment_item(appointment_doc, item):
	...
	item.amount = charge
	item.qty = 1
	item.reference_dt = "OT Schedule"
	item.reference_dn = appointment_doc.name
	return item


def cancel_appointment(appointment_id):
	appointment = frappe.get_doc("OT Schedule", appointment_id)
...
def check_sales_invoice_exists(appointment):
	sales_invoice = frappe.db.get_value(
		"Sales Invoice Item",
		{"reference_dt": "OT Schedule", "reference_dn": appointment.name},
		"parent",
	)
...
def update_status(appointment_id, status):
	frappe.db.set_value("OT Schedule", appointment_id, "status", status)
	...
	procedure_prescription = frappe.db.get_value(
		"OT Schedule", appointment_id, "procedure_prescription"
	)
...
def send_appointment_reminder():
	if frappe.db.get_single_value("Healthcare Settings", "send_appointment_reminder"):
		...
		for appointment in appointment_list:
			doc = frappe.get_doc("OT Schedule", appointment.name)
			message = frappe.db.get_single_value("Healthcare Settings", "appointment_reminder_msg")
			send_message(doc, message)
			frappe.db.set_value("OT Schedule", doc.name, "reminded", 1)
...
def get_events(start, end, filters=None):
	...
	conditions = get_event_conditions("OT Schedule", filters)

	data = frappe.db.sql(
		"""
		select
		`tabOT Schedule`.name, `tabOT Schedule`.patient,
		`tabOT Schedule`.practitioner, `tabOT Schedule`.status,
		`tabOT Schedule`.duration,
		timestamp(`tabOT Schedule`.appointment_date, `tabOT Schedule`.appointment_time) as 'start',
		`tabAppointment Type`.color
		from
		`tabOT Schedule`
		left join `tabAppointment Type` on `tabOT Schedule`.appointment_type=`tabAppointment Type`.name
		where
		(`tabOT Schedule`.appointment_date between %(start)s and %(end)s)
		and `tabOT Schedule`.status != 'Cancelled' and `tabOT Schedule`.docstatus < 2 {conditions}""".format(
...
def update_appointment_status():
	...
	appointments = frappe.get_all(
		"OT Schedule", {"status": ("not in", ["Closed", "Cancelled"])}, as_dict=1
	)

	for appointment in appointments:
		frappe.get_doc("OT Schedule", appointment.name).set_status()
